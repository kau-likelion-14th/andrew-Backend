...
public class TodoService {
    ...
    private final TodoDateRepository todoDateDateRepository;
...
// 투두 삭제
    ...
    public TodoDetailResponse createTodo(Long userId, TodoCreateRequest request, LocalDate date) {
        ...
        if(routineEnabled) {
            ...
            validateRoutineRequest(request.getStartDate(), request.getEndDate(), week);
        }
        ...
        if(routineEnabled) {
            ...
            todoDateDateRepository.save(todoDate);
        }
    ...
    public TodoDetailResponse updateTodoDetail(
        ...
        todo.update(request.getDescription(), routineEnabled, startDate, endDate, category, week);
    ...
    public TodoListResponse todoComplete(Long userId, Long todoId, LocalDate date, boolean completed) {
        ...
        TodoDate todoDate = todoDateDateRepository.findByTodo_IdAndDate(todoId, date)
                .orElseThrow(() -> new GeneralException(ErrorCode.TODO_DATE_NOT_FOUND));
        ...
        return TodoListResponse.from(todo, completed);
    }
}
